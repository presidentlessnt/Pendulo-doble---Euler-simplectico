...
def qGL(fun,a,b,N):
    ...
    k1=(b-a)/2
    k2=(b+a)/2
    return k1*sum(fun(k1*xi[j]+k2)*wi[j] for j in range(len(xi)))
...
def medo_rk4(mfun,myi,h,xi,xf):
    #el primer componente del array xmyi debe ser la variable indep
    myi=np.array(myi)
    step=int((xf-xi)/h)
    myn=np.zeros((step+1,len(myi)),dtype=np.float64)
    myn[0]=myi
